        ...
        <div v-if="showModal" class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" @click.self="closeModal">
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                ...
                <div class="p-8 space-y-6">
                    <!-- Prefix Selection (Super Admin) / Display (Prefix Admin) -->
...
const filters = reactive({ prefix: 'ALL', status: 'ALL', loginId: '', department: '' });
...
const filteredAccounts = computed(() => {
    ...
    return accounts.filter(acc => {
        ...
        return matchPrefix && matchStatus && matchLoginId && matchDept;
    });
});
...
const saveAccount = () => {
    ...
    // Validation
    if (!form.prefix || !form.username) {
        alert('Please fill in all required fields (Prefix and Login ID).');
        return;
    }
